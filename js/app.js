...
function renderSetup(error = "") {
  ...
  root.innerHTML = `<div class="setup-screen"><div class="card card-pad" style="max-width:42rem;text-align:left"><div class="center"><div class="brand" style="justify-content:center"><span class="brand-mark">B</span><span>BigGame</span></div><h1 style="margin-top:1.25rem">Connect Supabase</h1></div>${error ? `<p class="alert alert-error">${escapeHTML(error)}</p>` : ""}<p>The app is deployed, but its public Supabase configuration is missing.</p><ol class="small muted" style="line-height:1.8"><li>In Vercel, open <strong>Settings → Environment Variables</strong>.</li><li>Add <code>NEXT_PUBLIC_SUPABASE_URL</code> and <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code>.</li><li>Run <code>supabase/schema.sql</code> in the Supabase SQL editor.</li><li>Deploy the two Supabase Edge Functions using the README instructions.</li><li>Redeploy the Vercel project.</li></ol><p class="xsmall quiet" style="margin-bottom:0">For non-Vercel hosting, put the two public values in config.json. Never expose the service-role key or admin code.</p></div></div>`;
}

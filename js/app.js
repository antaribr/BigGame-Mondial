...
function renderSetup(error = "") {
  ...
  root.innerHTML = `<div class="setup-screen"><div class="card card-pad" style="max-width:42rem;text-align:left"><div class="center"><div class="brand" style="justify-content:center"><span class="brand-mark">B</span><span>BigGame</span></div><h1 style="margin-top:1.25rem">Connect Supabase</h1></div>${error ? `<p class="alert alert-error">${escapeHTML(error)}</p>` : ""}<p>The app is deployed, but its public Supabase configuration is missing.</p><ol class="small muted" style="line-height:1.8"><li>In Vercel, open <strong>Settings → Environment Variables</strong>.</li><li>Add <code>SUPABASE_URL</code> and <code>SUPABASE_ANON_KEY</code>.</li><li>Add the private server variables listed in README.md.</li><li>Run <code>supabase/schema.sql</code> in the Supabase SQL editor.</li><li>Redeploy the Vercel project.</li></ol><p class="xsmall quiet" style="margin-bottom:0">For plain static hosting, put the two public values in config.json. Never put the service-role key or admin code in a browser file.</p></div></div>`;
}

-- ============================================================================
-- BIGGAME VANILLA — complete Supabase schema for a NEW project
...
-- Numeric scores support the quiz's 0.5-point increments.
...
-- Freezes each attempt's random question set. Correct answers never go to the browser.
...
-- Recreate the view so its output types match numeric completion scores.
drop view if exists public.leaderboard;
create view public.leaderboard as
select
  t.id as team_id,
  t.name as team_name,
  count(distinct c.station_id)::integer as tasks_completed,
  coalesce(sum(c.score), 0::numeric)::numeric(12,2) as total_points,
  rank() over (
    order by coalesce(sum(c.score), 0::numeric) desc,
...
-- RLS: public reads power team/advisor screens. Public writes are limited to
-- registration. Scores require a valid station code through an RPC. Admin and
-- quiz writes happen only in Edge Functions using the service-role key.
...
-- Remove policies used by the older browser-graded quiz. The Edge Function now
-- protects correct answers and performs all quiz writes.
...
-- Team and station codes are shared credentials. Public list queries receive
-- only non-secret columns; exact-code lookups go through SECURITY DEFINER RPCs.
...
revoke select on table public.stations from anon, authenticated;
...
-- Remove the old integer overload, then create the numeric scoring RPC.
...
begin
  select * into v_station from public.stations where code = upper(trim(p_station_code));
...
-- Realtime for public live screens.
do $$
begin
